...
read-only, append-only ``auth_privileged_action_audit`` table and installs
...
- Any ``DELETE`` is rejected unless the current session variable
  ``@audit_purge_active`` is set to ``1`` — which only
  ``purge_expired_audit_rows`` (the horizon-bounded retention purge) ever
  does (setting it before, and clearing it after, its batched deletes, since
  MySQL/MariaDB session variables persist on the pooled connection past the
  transaction boundary) — so an ad-hoc/targeted single-row delete is rejected
  by the database itself, not merely absent from the application's API
  surface.
...
Revision ID: 528369529c1f
Revises: eaa7c094c234
...
revision: str = '528369529c1f'
down_revision: Union[str, None] = 'eaa7c094c234'
...
CREATE TRIGGER trg_auth_privileged_action_audit_no_update
BEFORE UPDATE ON auth_privileged_action_audit
FOR EACH ROW
BEGIN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'auth_privileged_action_audit rows are write-once and cannot be updated';
END;
"""

_NO_TARGETED_DELETE_TRIGGER_SQL = """
CREATE TRIGGER trg_auth_privileged_action_audit_no_delete
BEFORE DELETE ON auth_privileged_action_audit
FOR EACH ROW
BEGIN
    IF @audit_purge_active IS NULL OR @audit_purge_active <> 1 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'auth_privileged_action_audit rows can only be removed by the horizon-bounded retention purge';
    END IF;
END;
"""
...
def downgrade() -> None:
    op.execute(sa.text('DROP TRIGGER IF EXISTS trg_auth_privileged_action_audit_no_delete'))
    op.execute(sa.text('DROP TRIGGER IF EXISTS trg_auth_privileged_action_audit_no_update'))
    op.drop_index(
        op.f('ix_auth_privileged_action_audit_actor_user_id'),

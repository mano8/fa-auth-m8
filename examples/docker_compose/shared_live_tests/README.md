...
| `LIVE_TEST_AUTH_HEALTH_URL` | `http://localhost:9000/user/health/` (explicit `/health` URL; falls back to `LIVE_TEST_AUTH_BASE`/health when unset) |
...
| `LIVE_TEST_PRIVATE_API_CLIENT_ID` | issuer consumer id (`example-api`) — `X-Internal-Client` for fa-auth-m8 ≥ 1.0.0; enables the F06 legacy-detection check |
...
`LIVE_TEST_PRIVATE_API_CLIENT_ID` is the issuer's consumer id (`example-api` here) sent as `X-Internal-Client`; the hardened/quickstart/postgres/metrics/rs256/vault stacks now run a per-consumer issuer (`fa-auth-m8` ≥ 1.0.0, `PRIVATE_API_CONSUMERS` active), so set it together with `LIVE_TEST_PRIVATE_API_SECRET` to enable the F06 legacy-detection check (token-only must be rejected `401`).

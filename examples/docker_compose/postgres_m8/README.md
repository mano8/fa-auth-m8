...
**PostgreSQL 18** + **HS256** symmetric tokens + **stateful** token mode. No monitoring services.
...
| --- | --- | --- |
| `.env.example` → `.env` | Compose itself (`${VAR}` interpolation) and the engine init scripts | `DB_USER`, `DB_PASSWORD`, `AUTH_DB_USER`, `AUTH_DB_PASSWORD`, `API_DB_USER`, `API_DB_PASSWORD`, `REDIS_PASSWORD` |

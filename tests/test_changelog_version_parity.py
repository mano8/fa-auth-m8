...
`2.0.1` and `2.0.2` were both tagged and released with no `CHANGELOG.md`
...
def test_current_version_has_a_changelog_entry() -> None:
    """The version in `auth_user_service.__version__` must head a CHANGELOG entry."""
    ...
    assert __version__ in headings, (
        ...
        f"version (auth_user_service.__version__ = {__version__!r}); every "

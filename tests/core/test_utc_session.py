...
server is ``tests/integration/database/test_session_clock.py``, run by the
database-integration matrix's ``postgresql-europe-madrid`` leg.
"""
...
from auth_user_service.core import engine_sync
from auth_user_service.core.utc_session import (
    UTC_SESSION_SQL,
    pin_utc_session,
    set_session_utc,
)
...
def test_the_service_engine_is_pinned() -> None:
    ...
    # engine engine_sync.py builds and act exactly when it speaks PostgreSQL.
    assert "pin_utc_session(create_engine(" in inspect.getsource(engine_sync)
    engine: Any = engine_sync.engine
